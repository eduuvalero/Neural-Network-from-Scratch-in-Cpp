...
void NeuralNetwork::addLayer(int inputs, int outputs, Activation act){
    ...
    if (!layers.empty()) {
        ...
        if (inputs != expectedInputs) {
            throw std::invalid_argument(
                "NeuralNetwork::addLayer incompatible dimensions: expected " +
                std::to_string(expectedInputs) + " inputs, got " + std::to_string(inputs)
            );
        }
    }
...
void NeuralNetwork::save(const std::string& path){
    ...
    for (const Layer& l : layers) {
        ...
        if(!file){
            throw std::runtime_error("NeuralNetwork::save write error: rows");
        }

        int outputs = l.getWeights().getCols();
        file.write((const char*)&outputs, sizeof(int));
        if(!file){
            throw std::runtime_error("NeuralNetwork::save write error: cols");
        }

        Activation act = l.getAct();
        file.write((const char*)&act, sizeof(int));
        if(!file){
            throw std::runtime_error("NeuralNetwork::save write error: activation");
        }
        ...
        if(!file){
            throw std::runtime_error("NeuralNetwork::save write error: weights");
        }
        ...
        if(!file){
            throw std::runtime_error("NeuralNetwork::save write error: bias");
        }
    }
}
...
void NeuralNetwork::load(const std::string& path){
    ...
    if(!file || numLayers < 0){
        throw std::runtime_error("NeuralNetwork::load invalid model file: header");
    }
    ...
    for(int i = 0; i<numLayers; i++){
        ...
        if(!file){
            throw std::runtime_error("NeuralNetwork::load invalid model file: rows");
        }
        file.read((char*)&outputs, sizeof(int));
        if(!file){
            throw std::runtime_error("NeuralNetwork::load invalid model file: cols");
        }
        file.read((char*)&act, sizeof(int));
        if(!file){
            throw std::runtime_error("NeuralNetwork::load invalid model file: activation");
        }
        if(inputs <= 0 || outputs <= 0){
        ...
        if(!file){
            throw std::runtime_error("NeuralNetwork::load invalid model file: bias");
        }
        b.setData(bData);
